...
// Pin definitions
const uint8_t dc_pin = 0;    // DC - Data/Command, pin 5 on LCD.
const uint8_t rst_pin = 1;
const uint8_t sce_pin = 2;
...
static __IO GPIOx_ODR_t *gpioa_odr = (__IO GPIOx_ODR_t *)(GPIOA_BASE_RD + GPIO_ODR_OFFSET);
...
void LCD_init(void)
{
	...
	// LED D/C pin
	crl->fields.cnf0 = GPIO_CNF_GENERAL_OUT_PP;
	crl->fields.mode0 = GPIO_OUT_MODE_2MHz;
	
	// LED Reset pin
	crl->fields.cnf1 = GPIO_CNF_GENERAL_OUT_PP;
	crl->fields.mode1 = GPIO_OUT_MODE_2MHz;
	...
	// Reset LCD
	gpioa_odr->fields.odr1 = 0;
	for(;i < 3000000; ++i);
	gpioa_odr->fields.odr1 = 1;
...
void LCD_write(bool data_or_command, uint8_t data)
{
  ...
  gpioa_odr->fields.odr0 = data_or_command;
	
	//Send the data
  gpioa_odr->fields.odr2 = 0;
  spi1_transmit(&data, 1);
  gpioa_odr->fields.odr2 = 1;
}
...
void update_display(void)
{
  ...
  for (i=0; i < (LCD_WIDTH * (LCD_HEIGHT / 8)); i++)
  {
    LCD_write(LCD_DATA, displayMap[i]);
  }
}
...
{
	int i;
  while (*string != 0x00) // loop until null terminator
  {
    set_char(*string++, x, y, bw);
    x+=5;
    for (i=y; i<y+8; i++)
    {
      set_pixel(x, i, !bw);
...
void set_line(int x0, int y0, int x1, int y1, bool bw)
{
  ...
  dy <<= 1; // dy is now 2*dy
  dx <<= 1; // dx is now 2*dx
  set_pixel(x0, y0, bw); // Draw the first pixel.
...
void set_rect(int x0, int y0, int x1, int y1, bool fill, bool bw)
{
  ...
  else
  {
    ...
    set_line(x0, y0, x1, y0, bw);
    set_line(x0, y1, x1, y1, bw);
    set_line(x0, y0, x0, y1, bw);
    set_line(x1, y0, x1, y1, bw);
  }
}
...
void set_contrast(uint8_t contrast)
{
  ...
  LCD_write(LCD_COMMAND, 0x80 | contrast); //Set LCD Vop (Contrast): Try 0xB1(good @ 3.3V) or 0xBF if your display is too dark
